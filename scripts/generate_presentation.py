...
def build() -> Path:
    ...
    _add_bullets(
        ...
        [
            ...
            "",
            "• FR-v2：預設 1 km、300 家；十五類美食分類 + 平價／avg_spend_ntd 篩選",
            "• FR-v2：營業狀態顯示；轉盤排除休息中（國定假日 specialDays 支援）",
            "",
            "【品質目標】",
            "• 109 項 pytest 全綠，coverage ≥ 84%",
    ...
    flow = (
        ...
        "[places_cache.public.json] ──雲端 300 家──┤\n"
    ...
    _add_bullets(
        ...
        [
            ...
            "Step 3 — 資料管線：Google Places 格網抓取 300 家 → enrich 十五類 → JSON 離線快取",
            "Step 4 — Streamlit UI：側欄分組分類 + 預算篩選 + 地圖互動",
            "Step 5 — 轉盤：營業中 Top 30 + SVG 扇形盤",
            "Step 6 — 品質保證：109 tests、integrity SHA256、overlay 人工校正",
    ...
    _add_table_slide(
        ...
        ["分頁", "功能", "技術細節"],
        [
            ["🗺️ 美食地圖", "餐廳清單 + 互動地圖", "預設 1 km／300 家；營業狀態欄"],
            ["", "美食分類（分組）", "十五類 multiselect + 其他勾選"],
            ["", "預算篩選", "price_level 1 或 avg_spend_ntd ≤ 上限"],
    ...
    _add_bullets(
        ...
        [
            ...
            "",
            "【測試】pytest -q --cov=src --cov-fail-under=70  →  109 passed, ~84% coverage",
            "",
            "【成果】宜大 1 km 內 300 家 · 十五類 · 平價篩選 · 轉盤 · CI 全綠",
        ],
        size=18,
    )
